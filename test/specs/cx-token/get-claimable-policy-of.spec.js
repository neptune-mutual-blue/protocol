...
describe('cxToken: `getClaimablePolicyOf` function', () => {
  let cxToken, deployed, coverKey, amountToCover
  ...
  beforeEach(async () => {
    ...
    amountToCover = helper.ether(100_000, PRECISION)
    ...
    // const values = [stakeWithFee, initialReassuranceAmount, minStakeToReport, reportingPeriod, cooldownPeriod, claimPeriod, floor, ceiling, reassuranceRate, leverageFactor]
  ...
  it('must correctly give the coverage starts from amount', async () => {
    ...
    const claimable = await cxToken.getClaimablePolicyOf(owner.address)
    claimable.should.equal(helper.ether(100_000))
  ...
  it('must return zero when no policies are purchased', async () => {
    ...
    const claimable = await cxToken.getClaimablePolicyOf(bob.address)
    await claimable.should.equal(0)
  })
})
