...
const add = async (coverKey, payload) => {
  ...
  const amount = ether(getRandomNumber(1_250_000, 5_000_000), 6)
  ...
  console.info('Adding %s to the vault. Stake: %s. From: %s', weiAsToken(amount, 'DAI', 6), weiAsToken(stake, 'NPM'), lp.address)

  await vault.addLiquidity(coverKey, amount, stake, toBytes32(''))

  console.info('Added %s to the vault. Stake: %s', weiAsToken(amount, 'DAI', 6), weiAsToken(stake, 'NPM'))
}
